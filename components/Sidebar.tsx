...
export default function Sidebar() {
  ...
  const DashboardIcon = ({ filled }: { filled?: boolean }) => (
    <svg className="w-5 h-5 flex-shrink-0" fill={filled ? 'currentColor' : 'none'} viewBox="0 0 24 24" stroke="currentColor" strokeWidth={filled ? 0 : 2}>
      <path strokeLinecap="round" strokeLinejoin="round" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />
    </svg>
  );

  const ClientsIcon = ({ filled }: { filled?: boolean }) => (
    <svg className="w-5 h-5 flex-shrink-0" fill={filled ? 'currentColor' : 'none'} viewBox="0 0 24 24" stroke="currentColor" strokeWidth={filled ? 0 : 2}>
  ...
  return (
    <>
      {/* Desktop sidebar */}
      <aside className="hidden md:flex w-60 bg-gray-950 min-h-screen flex-col text-white flex-shrink-0">
        {/* Logo */}
        ...
        <nav className="p-3 border-b border-gray-800 space-y-0.5">
          {navLink('/', 'Dashboard', <DashboardIcon />)}
          {navLink('/clients', 'Clients', <ClientsIcon />)}
        </nav>
      ...
      {/* Mobile bottom nav */}
      <nav className="md:hidden fixed bottom-0 inset-x-0 z-40 bg-gray-950 border-t border-gray-800">
        <div className="flex">
          <Link
            href="/"
          ...
          >
            <DashboardIcon filled={isActive('/')} />
            Dashboard
          </Link>
          <Link
            href="/clients"
            className={`flex-1 flex flex-col items-center gap-1 py-3 text-xs font-medium transition-colors ${
              isActive('/clients') ? 'text-white' : 'text-gray-500'
            }`}
          >
            <ClientsIcon filled={isActive('/clients')} />
            Clients
          </Link>
        </div>
      </nav>
